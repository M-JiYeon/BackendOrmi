...
# def index(request): # request 가 get인지 post인지
...
# model, template_name, context_objext_name,
...
class Write(LoginRequiredMixin, View):
    ...
    def post(self, request): # requdst -> HttpRequest 객체
        form = PostForm(request.POST)
        if form.is_valid():
            post = form.save(commit=False) # commit=False 변수 할당만 우선 하고 이후에 수정가능
            post.writer = request.user
            post.save()
            return redirect('blog:list') # response -> HttpRequest 객체
...
class HashTagWrite(View):
    def post(self, request, pk): # pk = post_id
        ...
        if form.is_valid():
            ...
            # 유저 정보 가져오기
            writer = request.user
            # 태그 객체 생성,, create 메서드를 사용할 때는 save 필요 없음
...
class HashTagDelete(View):
    def post(self, request, pk): # pk = hashtag_id
        # 해시태그 불러오기
        hashtag = HashTag.objects.get(pk=pk)
        # 포스트의 pk(id) 값
        hashtag_id = hashtag.post.id
        # 해시태그삭제
        hashtag.delete()
        # 응답
