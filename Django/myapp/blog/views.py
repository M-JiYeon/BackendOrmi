...
#     def get_success_url(self): # get_absolute_url'title': 'Blog'
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            ...
            try:
                ...
                # 생성할 것이 이미 있다면 오류 발생, Unique 값이 중복될 때
                # 필드 값이 비어있을 때: ValidationError
                # 외래키 관련 데이터베이스 오류: ObjectDoesNotExist
                # get_or_create() -> 2가지 경우의 리턴값
                # comment, create = comment = Comment.objects.get_or_create(post=post, content=content, writer=writer)
